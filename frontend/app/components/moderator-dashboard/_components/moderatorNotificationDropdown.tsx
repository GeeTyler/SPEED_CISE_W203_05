...
const ModeratorNotificationDropdown: React.FC = () => {
    const notifications = useNotifications('submission');
  ...
  return (
    <div className="relative inline-block">
      <button
        className="relative bg-blue-500 text-white rounded-full w-8 h-8 flex items-center justify-center"
        onClick={() => setIsOpen(!isOpen)}
      >
        !
        {unreadCount > 0 && (
          <span className="absolute top-0 right-0 bg-white text-blue-500 rounded-full w-4 h-4 text-xs flex items-center justify-center">
            {unreadCount}
          </span>
        )}
      </button>
